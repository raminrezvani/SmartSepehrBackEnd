...
# ========= Read All hotels name (from Eghamat24) ====
from .lstHotels_AZD_withProperty import yazd_hotels
from .lstHotels_GSM_withProperty import gheshm_hotels
from .lstHotels_IFN_withProperty import isfahan_hotels
from .lstHotels_KIH_withProperty import kish_hotels
from .lstHotels_MHD_withProperty import mashhad_hotels
from .lstHotels_SYZ_withProperty import shiraz_hotels
from .lstHotels_TBZ_withProperty import tabriz_hotels
from .lstHotels_THR_withProperty import tehran_hotels
...
# set([a['destination'] for a in HotelRooms])
# HotelRooms[]
...
#====================
HotelRooms_eghamat24={}
HotelRooms_eghamat24['MHD']=mashhad_hotels
HotelRooms_eghamat24['THR']=tehran_hotels
HotelRooms_eghamat24['AZD']=yazd_hotels
HotelRooms_eghamat24['IFN']=isfahan_hotels
HotelRooms_eghamat24['SYZ']=shiraz_hotels
HotelRooms_eghamat24['TBZ']=tabriz_hotels
...
class DNS_mapping:
    ...
    def check_hotelName(self,text,destination):
        ...
        #     # return matched
        # elif destination=="GSM":
        #     #==== preprrocess
        #
        #     text=text.replace('آرتیمیس','آرتميس')
        #     #===
        #
        #     for i in range(0,len(self.list_hotels_ghesm)):
        #         spl=self.list_hotels_ghesm[i].split(' ')
        #         cnt=0
        #         for j in range(0,len(spl)):
        #             if (spl[j] in text):
        #                 cnt=cnt+1
        ...
        #             if (' ارم' in text or
        #             'ارم ' in text) and ('گاردن' not in text):
        #                 return  'هتل '+self.list_hotels_ghesm[i]+' قشم'
        #             return 'هتل '+self.list_hotels_ghesm[i]+' قشم'
        #

        #=======================
        # from Mojalal24.ir Hotel Room names
        #=====================
        # bayad dasti jolo beravim
        # else:  # gheyr az KIH and GSM:
        ...
        #=====
        # برای این باید کلمه هتل و کلمه اصفهان را برداشت تا دچار مشکل نشود
        #==========
        ...
        try:
            ...
            # text = re.sub(fr'\b{mapping_name.get(destination, destination)}\b', '', text).strip()
        ...
        #
        # #================ OLD =========
        # # Zip the 'hotels' and 'star' lists together, sort by the length of 'hotels' in descending order, and unzip back
        # sorted_pairs = sorted(
        #     zip(lst_hotels_mojalal['hotels'], lst_hotels_mojalal['stars']),
        ...
        if (len(lst_hotels_mojalal)!=0):
            for i in range(0,len(lst_hotels_mojalal['hotels'])):
                ...
                # if ('بهشتیان' in lst_hotels_mojalal['hotels'][i]):
                #     print('asdasd')
                ...
                # --- split from هتل ----
                dns_text=lst_hotels_mojalal['hotels'][i]
                for s in lst_split:
                ...
                import re
                try:
                    # dns_text = re.sub(fr'\b{mapping_name.get(destination, destination)}\b', '', dns_text).strip()
                ...
                if(cnt==len(spl)):
                    ...
                    #===================
                    # print(lst_hotels_mojalal[i])

                    # if (lst_hotels_mojalal['hotels'][i]=='هتل سنتی اصفهان'):
                    #     print('asdasdasd')
        ...
        #
        # #======= Check eghamat24 hotelNames ================
        # lst_hotels_eghamat=HotelRooms_eghamat24.get(destination,[])
        # for i in range(0,len(lst_hotels_eghamat)):
        #     if (lst_hotels_eghamat[i]==''):
        #         continue
        #     spl=lst_hotels_eghamat[i]['title'].split(' ')
        #     cnt=0
        #     for j in range(0,len(spl)):
        #         if (' '+spl[j]+' ' in text or
        #                 spl[j]+' ' in text or
        ...
        #             cnt=cnt+1
        #     if(cnt==len(spl)):
        #         return lst_hotels_eghamat[i]['title']
        # #==========================================
        #
        # #== New Hotel Name
        # return text
