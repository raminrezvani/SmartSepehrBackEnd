...
class TourCollector:
    def __init__(self, start_date, night_count, hotel_star, adults=2):
    ...
    def get_single_data(self, start_date):
        ...
        print("--------------------------------")
        print("booking start")
        booking_instance = Booking(start_date, self.night_count, adults=self.adults)
        booking = self.executor.submit(booking_instance.get_result)
        print("--------------------------------")
        print("dayan start")
        dayan_instance = Sepehr("KIH", start_date, self.night_count, cookie_data.DAYAN, "dayan", adults=self.adults)
        dayan = self.executor.submit(dayan_instance.get_result)
        print("--------------------------------")
        print("deltaban start")
        deltaban_instance = Deltaban(start_date, self.night_count, adults=self.adults)
        deltaban = self.executor.submit(deltaban_instance.get_result)
        print("--------------------------------")
        print("sepid parvaz start")
        sepid_parvaz_instance = Sepehr("KIH", start_date, self.night_count, cookie_data.SEPID_PARVAZ, "sepid_parvaz",
                                       adults=self.adults)
        sepid_parvaz = self.executor.submit(sepid_parvaz_instance.get_result)
        print("--------------------------------")
        print("mehrab seir start")
        mehrab_instance = Sepehr("KIH", start_date, self.night_count, cookie_data.MEHRAB, "mehrab", adults=self.adults)
        mehrab = self.executor.submit(mehrab_instance.get_result)
        print("--------------------------------")
        print("rahbal start")
        rahbal_instance = Sepehr("KIH", start_date, self.night_count, cookie_data.RAHBAL, "rahbal", adults=self.adults)
        rahbal = self.executor.submit(rahbal_instance.get_result)
        print("--------------------------------")
        print("tak setare start")
        tak_setare_instance = Sepehr("KIH", start_date, self.night_count, cookie_data.TAK_SETAREH, "tak_setare",
                                     adults=self.adults)
        tak_setare = self.executor.submit(tak_setare_instance.get_result)
        print("--------------------------------")
        print("omid oj start")
        omid_oj_instance = Sepehr("KIH", start_date, self.night_count, cookie_data.OMID_OJ, "omid_oj",
                                  adults=self.adults)
        omid_oj = self.executor.submit(omid_oj_instance.get_result)
        print("--------------------------------")
        print("parmis start")
        parmis_instance = Sepehr("KIH", start_date, self.night_count, cookie_data.PARMIS, "parmis", self.adults)
        ...
        # ---
        try:
            deltaban = deltaban.result(timeout=400)
        except TimeoutError as e:
            print("--------------------------------")
        ...
        # # ---
        try:
            alwin = alwin.result(timeout=10000)
        ...
        # ---
        try:
            booking = booking.result(timeout=600)
        except TimeoutError as e:
            print("--------------------------------")
    ...
    def get_single_result(self, start_date=None, show_providers=False, use_cache=True):
        if start_date:
    ...
    def get_analysis(self, range_number: int, use_cache=True):
